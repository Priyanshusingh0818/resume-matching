...
import { Mail, Lock, ChevronRight, GraduationCap, Loader2, AlertCircle, Sparkles } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10 animate-fade-in">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-3xl font-bold text-gray-100">Student Login</h1>
          <p className="text-gray-500">Your career intelligence starts here</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="glass-card p-8 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="student-email" className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Email</label>
              <div className="relative">
                <Mail className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" />
                <input id="student-email" type="email" value={email} onChange={e => setEmail(e.target.value)} disabled={loading}
                  className="input-field pl-11" placeholder="name@email.com" required autoComplete="email" />
              </div>
            </div>
            <div>
              <label htmlFor="student-password" className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Password</label>
              <div className="relative">
                <Lock className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" />
                <input id="student-password" type="password" value={password} onChange={e => setPassword(e.target.value)} disabled={loading}
                  className="input-field pl-11" placeholder="••••••••" required autoComplete="current-password" />
              </div>
            </div>
          </div>

          <button id="student-login-btn" type="submit" disabled={loading}
        ...
        <div className="text-center space-y-3">
          <p className="text-gray-500 text-sm">
            Don't have an account? <Link to="/student/register" className="text-primary-400 font-semibold hover:underline">Register</Link>
          </p>
          <p className="text-gray-600 text-xs">
            Recruiter? <Link to="/admin/login" className="text-primary-400 font-semibold hover:underline">Admin Portal</Link>
          </p>
        </div>
      </div>
    </div>
